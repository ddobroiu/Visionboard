                            ...
                            <div className="p-4 sm:p-5 space-y-4">
                                ...
                                <div className="space-y-3 pt-2">
                                    ...
                                    <label className="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-300">
                                        ...
                                        />
                                        <span>
                                            Doresc să primesc oferte și noutăți de la Visionboard.ro (fără
                                            spam).
                                        </span>
                                    </label>
                                </div>
...
function CartItems({
    ...
    // Render details removed simplification for brevity as it was very long, but kept essential structure.
    // Actually, I should probably copy the renderDetails logic too to be safe.
    ...
    function renderDetails(item: any) {
        ...
        return (
            <div className="mt-2 rounded-md border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 p-2 text-xs text-slate-700 dark:text-slate-300">
                {Object.entries(meta).map(([k, v]) => {
                    if (typeof v === 'object') return null;
                    return (
                        <div key={k} className="flex gap-2 py-0.5">
                            <span className="opacity-90 font-medium">{k}:</span>
    ...
    return (
        <div className="rounded-2xl border-2 border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:p-5 shadow-md">
            ...
            <ul className="divide-y divide-slate-200 dark:divide-slate-700">
                {(items ?? []).map((item) => {
                    const title = item.title || item.name || "Produs";
                    ...
                    return (
                        <li key={item.id} className="py-3 first:pt-0 last:pb-0">
                            <div className="flex items-start gap-3">
                                <div className="h-14 w-14 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center border border-slate-200 dark:border-slate-700">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex gap-2 justify-between">
                                        <div className="flex items-center gap-3 flex-1 min-w-0">
                                            ...
                                            <p className="font-black text-base sm:text-lg text-black dark:text-white leading-snug flex-1 min-w-0">
                                                {title}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-2 shrink-0">
                                            <span className="text-sm font-bold">{qty} buc.</span>
                                        </div>
                                    </div>
                                    {renderDetails(item)}
                                    <div className="flex justify-between items-center mt-1.5">
                                        <span className="text-xs text-slate-700 dark:text-slate-300">Preț unitar</span>
                                        <span className="text-sm font-semibold text-slate-900 dark:text-white">{fmt(unit)}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs text-slate-700 dark:text-slate-300">Total produs</span>
                                        <span className="text-base font-bold text-black dark:text-white">{fmt(unit * qty)}</span>
                                    </div>
                                </div>
                            </div>
                        </li>
