...
export async function GET(request: Request) {
    ...
    try {
        ...
        const response = await fetch(`https://pixabay.com/api/?key=${apiKey}&q=${encodeURIComponent(query)}&image_type=photo&per_page=40&safesearch=true&lang=ro${transparentParam}`);
        let data = await response.json();

        // If no results in Romanian, try English search as a fallback
        if (!data.hits || data.hits.length === 0) {
            const fallbackResponse = await fetch(`https://pixabay.com/api/?key=${apiKey}&q=${encodeURIComponent(query)}&image_type=photo&per_page=40&safesearch=true${transparentParam}`);
